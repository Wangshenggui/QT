...
from sensor_msgs.msg import Image  # 用于接收图像消息
...
class MyDialog(QtWidgets.QDialog, Ui_Dialog):
    def __init__(self):
        ...
        # 订阅 '/camera/depth/image_raw' 话题，接收图像数据
        self.image_subscriber = rospy.Subscriber('/camera/rgb/image_raw', Image, self.image_callback)

        # 初始化 CvBridge，用于转换 ROS 图像消息为 OpenCV 格式
        self.bridge = CvBridge()

        # 初始化图像显示组件
        self.image_label = QtWidgets.QLabel(self)  # 创建 QLabel 用于显示图像
        self.image_label.setGeometry(500, 400, 600, 360)  # 设置位置和大小，确保不与其他文本和组件重叠
        self.image_label.setAlignment(QtCore.Qt.AlignCenter)  # 设置文本居中
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if len(cv_image.shape) == 3:
                ...
                q_image = QtGui.QImage(cv_image.data, width, height, bytes_per_line, QtGui.QImage.Format_Grayscale8)
    ...
    def create_number_pad(self):
        ...
        # 创建按钮列表
        button_list = [
            ('1', 100, 650), ('2', 200, 650), ('3', 300, 650),
            ('4', 100, 700), ('5', 200, 700), ('6', 300, 700),
            ('7', 100, 750), ('8', 200, 750), ('9', 300, 750),
            ('0', 200, 800), ('Del', 100, 800), ('Clr', 300, 800)
        ]
        
        # 创建并显示数字按钮
        for text, x, y in button_list:
            button = QtWidgets.QPushButton(text, self)
            button.setGeometry(x, y, 80, 40)
            button.clicked.connect(lambda checked, text=text: self.on_number_button_click(text))
    ...
    def on_number_button_click(self, text):
        ...
        if text == 'Del':
            # 删除最后一个字符
            self.phone_input.setText(current_text[:-1])
        elif text == 'Clr':
            # 清空输入框
            self.phone_input.clear()
        else:
            # 添加数字到输入框
            self.phone_input.setText(current_text + text)
    ...
    def send_room_to_usb1(self):
        """
        当火灾发生时，通过 ttyUSB1 发送特定的指令
        """
        if self.ser_usb1 and self.current_room is not None:
            try:
                # 创建要发送的字节指令 (7E 05 41 00 01 45 EF)
                command = bytearray([0x7E, 0x04, 0x33, 0x02, 0x35, 0xEF])
                command2 = bytearray([0x7E, 0x05, 0x41, 0x00, 0x01, 0x45, 0xEF])
                # 打印发送的字节数据
                print(f"发送指令: {command.hex().upper()}")
                # 发送字节流到串口
                self.ser_usb1.write(command)
                time.sleep(0.1)
                self.ser_usb1.write(command2)
                self.ser_usb1.flush()  # 确保缓冲区数据已经发送
                print(f"已向串口 {self.serial_port_usb1} 发送指令: {command.hex().upper()}")
            except Exception as e:
                print(f"发送命令时发生错误: {e}")
    ...
    def send_to_external_server(self):
        """
        向外部服务器发送房间号和电话号码
        """
        if self.current_room is not None and self.saved_phone_number:
            try:
                # 连接到外部服务器
                external_server = ('47.109.46.41', 9999)
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
                    client_socket.connect(external_server)
                    message = {
                        "RoomNum": self.current_room,
                        "PhoneNum": self.saved_phone_number
                    }
                    client_socket.sendall(json.dumps(message).encode())  # 发送消息
                    print(f"已向 {external_server} 发送数据: {message}")
            except Exception as e:
                print(f"向外部服务器发送数据时发生错误: {e}")
